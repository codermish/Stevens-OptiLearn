...
/* Force any nested text or icons to stay white */
button, button * {
  color: white !important;
}
...
/* Slider marks (0, 100, etc) styling */
.rc-slider-mark-text {
  background: linear-gradient(135deg, #8B1538 0%, #A91B47 100%) !important;
  color: white !important;
  ...
  transform: translateY(-50%) !important; /* vertically center against rail */
}

/* If you want the active mark (when the handle is exactly on it) to look identical */
...
# MongoDB connection
client = MongoClient("mongodb+srv://codermishyt:tQk0U81QVvktb5s4@optilearn.vu626pu.mongodb.net/")
db = client.otpilearn
...
def save_recommendations_to_company(company_name, recommendations, user_requirements_text=None):
    """
    Add course recommendations to existing company document
    
    Args:
        company_name (str): Name of the company
        recommendations (list): List of course recommendation dictionaries
        user_requirements_text (str): Original user requirements text
    """
    try:
        ...
        for course in recommendations:
            recommendation_item = {
                "rank": course.get('rank', 0),
                "course_code": course.get('course_code', 'N/A'),
                "course_name": course.get('course_name', 'Untitled Course'),
                "description": course.get('description', 'No description available.'),
                "similarity_score": course.get('similarity_score', 0),
                "adjusted_similarity": course.get('adjusted_similarity', 0),
                "similarity_percentage": convert_similarity_to_percentage(course.get('adjusted_similarity', 0)),
                "original_similarity_percentage": convert_similarity_to_percentage(course.get('similarity_score', 0)),
                "course_type": course.get('course_type', 'mixed'),
                "best_week": course.get('best_week', 'overview'),
                "module_title": course.get('module_title', ''),
                "learning_outcomes": course.get('learning_outcomes', [])
            }
            recommendations_data["recommendations"].append(recommendation_item)
        
        # Update the company document with recommendations
        result = collection.update_one(
            {"company_name": company_name},
            {"$set": {"course_recommendations": recommendations_data}}
        )
        
        if result.modified_count > 0:
            st.success(f"✅ Recommendations saved for {company_name}")
            return True
        else:
            st.warning(f"⚠️ No changes made to {company_name}")
            return False
...
def get_company_recommendations(company_name):
    """
    Retrieve course recommendations for a specific company
    
    Args:
        company_name (str): Name of the company
    
    Returns:
        dict: Recommendations data or None if not found
    """
    try:
        company = collection.find_one(
            {"company_name": company_name}, 
            {"course_recommendations": 1, "company_name": 1}
        )
        
        if company and "course_recommendations" in company:
            return company["course_recommendations"]
        else:
            return None
...
def get_current_company_name():
    """
    Get company name from session state form data
    """
    form_data = st.session_state.get('form_data', {})
    return form_data.get('company_name')
